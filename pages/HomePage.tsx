...
const HomePage: React.FC = () => {
    const { artists, news, trendingTracks, playPlaylist, heroBackgroundImage, portalUrl } = useContent();
    ...
    const handlePlayRadio = () => {
        ...
        const freeStreamableTracks = allTracks.filter(t => 
            (!t.accessTier || t.accessTier === SubscriptionTier.FREE) && isStreamableUrl(t.sourceUrl)
        );

        if(freeStreamableTracks.length === 0) {
            toast.error("No streamable tracks available for the radio right now.");
            return;
        };

        // Shuffle the tracks to create a dynamic playlist
        const shuffledTracks = [...freeStreamableTracks].sort(() => Math.random() - 0.5);

        const radioPlaylist: CurrentlyPlayingTrack[] = shuffledTracks.map(t => ({
            ...t,
            artistName: t.artist.name,
            releaseTitle: t.release.title,
            coverImageUrl: t.release.coverImageUrl,
        }));
        
        playPlaylist(radioPlaylist);
        toast.success("S.M.T. RADIO is now live!");
    };
            ...
            >
                ...
                <div className="relative z-10 p-4">
                    ...
                    <div className="flex items-center justify-center space-x-8 mb-16">
                        <HeroButton 
                            label="S.M.T. RADIO" 
                            icon={<PlayIcon className="w-10 h-10 text-white"/>} 
